...
class UserCreateForm(FlaskForm):
    ...
    address = StringField('주소', validators=[DataRequired('주소를 입력해주세요')])
...
class ProductForm(FlaskForm):
    ...
    price = IntegerField('가격',
                         validators=[DataRequired('기격을 입력해주세요.'), NumberRange(min=0, message='가격은 0원 이상이어야 합니다.')])
    images = MultipleFileField('이미지 URL',
                               validators=[FileAllowed(['jpg', 'jpeg', 'png', 'gif', 'wedp'], '이미지 파일만 업로드 가능합니다.')])
